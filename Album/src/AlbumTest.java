...
public class AlbumTest {
    ...
    @Test
    public void testarDuasInstanciasDeAlbumDeTiposDiferentes() {

        Repositorio repositorioFigurinhas = new Repositorio("album_copa2014", TAMANHO_DO_ALBUM);
        Repositorio repositorioDeSelos = new Repositorio("selinkk", TAMANHO_DO_ALBUM);
        Album albumFigurinha = new Album<Figurinha>(repositorioFigurinhas, ITENS_POR_PACOTE);
        Album albumSelos = new Album<Selo>(repositorioDeSelos, ITENS_POR_PACOTE);

        Pacotinho pacoteSelos = new Pacotinho(repositorioDeSelos, new int[]{24,69,121});
        Pacotinho pacoteSelos2 = new Pacotinho(repositorioDeSelos, new int[]{4,23,5});
        albumSelos.receberNovoPacotinho(pacoteSelos);
        albumSelos.receberNovoPacotinho(pacoteSelos2);
        Pacotinho pacotedeFigurinha = new Pacotinho(repositorioFigurinhas, new int[]{42,96,121});
        albumFigurinha.receberNovoPacotinho(pacotedeFigurinha);
        assertEquals(3,albumFigurinha.getQuantItensColados());
        assertEquals(6,albumSelos.getQuantItensColados());
    }
