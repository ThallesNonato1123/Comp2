...
//        tuite.anexarAlgo(new Object());  // essa linha, se fosse descomentada, daria erro de compilação
    ...
    @Test
    public void testeExcecoes() throws UsuarioDesconhecidoException, TamanhoMaximoExcedidoException {

        try {
            tuiterLite.tuitarAlgo(usuario, null);

        } catch (IllegalArgumentException | UsuarioDesconhecidoException | TamanhoMaximoExcedidoException e) {
            assertEquals("Não possivel", e.getMessage());
        }
    }

    @Test
    public void testeTamanho() throws UsuarioDesconhecidoException, TamanhoMaximoExcedidoException {
        try {
            StringBuilder builder = new StringBuilder();
            builder.append("teste".repeat(121));

            tuiterLite.tuitarAlgo(usuario, builder.toString());
        } catch (UsuarioDesconhecidoException | TamanhoMaximoExcedidoException e) {
            assertEquals("Ultrapassou tamanho maximo", e.getMessage());
        }

    }

    @Test
    public void testeUserDesconheicdo() throws UsuarioDesconhecidoException, TamanhoMaximoExcedidoException {
        try {
            tuiterLite.tuitarAlgo(new Usuario("UsuarioTeste", "usuarioteste@gmail.com"), "teste");
        } catch (UsuarioDesconhecidoException | TamanhoMaximoExcedidoException e) {
            assertEquals("Usuario desconhecido", e.getMessage());
        }
    }
