...
public class TuiterLite<T> {
    ...
    public Tuite<T> tuitarAlgo(Usuario usuario, String texto) throws UsuarioDesconhecidoException, TamanhoMaximoExcedidoException {
       ...
       // if (texto == null || texto.length() == 0 || texto.length() > TAMANHO_MAXIMO_TUITES ||
        ...
        if(!this.usuarioByEmail.containsKey(usuario.getEmail())) throw new UsuarioDesconhecidoException("Usuario desconhecido");
        if (texto == null || texto.length() == 0 || usuario == null) throw new IllegalArgumentException("Não possivel");
